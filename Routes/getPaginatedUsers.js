...
router.route("/").post(authenticateAdmin, async (req, res) => {
  try {
    ...
    const users = await db.query(
        `
        SELECT u.id, u.first_name, u.last_name, u.last_name, u.phone_number, u.email, u.created_at, u.created_by_id, u.token, r.name as role, r.id as roleId, s.name as status, s.id as statusId, p.name as position, p.id as positionId
