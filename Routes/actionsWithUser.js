...
router.route("/update-user").post(authenticateAdmin, async (req, res) => {
  try {
      await db.query(`update users set first_name = '${req.body.firstName}', last_name = '${req.body.lastName}', birthday = '${req.body.birthday}', email = '${req.body.email}', phone_number = '${req.body.phoneNumber}', position_id = '${req.body.positionId}', role_id = '${req.body.roleId}', status_id = '${req.body.statusId}', updated_at = to_timestamp(${Date.now()} / 1000.0) where id = ${req.body.id}`);
